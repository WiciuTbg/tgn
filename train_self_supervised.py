...
from utils.neighbor_finder import RandEdgeSampler, get_neighbor_finder
...
for epoch in range(NUM_EPOCH):
  ...
  for batch_idx in range(0, num_batch):
    ...
    # Detach memory after batch so we don't backpropagate to
    # the start of time
  ...
  nn_val_ap, nn_val_auc = eval_edge_prediction(model=tgn, negative_edge_sampler=val_rand_sampler, data=new_node_val_data, n_neighbors=NUM_NEIGHBORS)
...
nn_test_ap, nn_test_auc = eval_edge_prediction(model=tgn, negative_edge_sampler=nn_test_rand_sampler, data=new_node_test_data, n_neighbors=NUM_NEIGHBORS)
